...
reservas = pd.DataFrame(reservas.iloc[:,-1].values,columns=['Spot'],index=reservas.iloc[:,0].values)
...
hoy = dt.date.today()
fechas = []
for i in range(12): #Changed to 11 as 12 sometimes dont have data
    if (hoy.month + i) > 12:
...
# handle BCRA dollar as the spot
...
#spot.index = indice
spot = spot.loc[f'{dolar.index[0]}':]


dolar.index = [dt.datetime.strptime(i,"%Y-%m-%d %H:%M") for i in dolar.index.to_list()]
# concat spot with the rest of the futures
dolar = pd.concat([spot,dolar]).sort_index()

dolar = dolar.fillna(method='ffill')

dolar.index = dolar.index.astype(str)
cierre = dolar.copy()
columnas = []
