...
monetaria['pases'] = herramientas.pases.fillna(method="ffill")
monetaria['leliqs'] = herramientas.leliqs.fillna(method="ffill")
monetaria['legar'] = herramientas.legar.fillna(method="ffill")
monetaria['stock_reservas'] = reservorio.stock_reservas.fillna(method="ffill")
...
columnas = list(depositos.iloc[4,:].values)    
...
depositos.index,depositos.columns = depositos.iloc[:,0].values,columnas    
#del depositos['Gerencia de Estadísticas Monetarias']
#depositos = (depositos.reset_index()
#            .drop_duplicates(subset='index', keep='last')
#            .set_index('index').sort_index())
depositos = depositos.head(length)
depositos.index = [i.strftime('%Y-%m-%d') for i in depositos.index.to_list()]    
depositos = depositos.loc[f'{start}':]
...
depositado['M2'] = depositos['M2(6)'].values
depositado,depositos = depositado.tail(length), depositos.tail(length)  
...
monetaria['Encaje_Bancario'] = (depositos.iloc[:,-4].values) * 0.32
monetaria['SWAP'] = 20000.0
