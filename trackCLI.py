...
"""4 main functions to handle the cycle of portfolio management FROM THE CLI
  ...
  this function calls AdjustRisk to perform CVaR analysis
...
def PortfolioMonitor(data):
  ...
  portfolio['priceToday'] = (yahoo.download(stocks,period="20d",interval="2m",prepost=True)['Adj Close'].fillna(method='ffill')).tail(1).T
...
def AdjustRisk(data):
  ...
  result[result.MinCVaR>=0.07] = 0.07 # ensures bounds limit
...
def portfolioRiskUpdated(data):
  ...
  df['weights'] = data.weights.to_list()
  ...
  df['weightsNew'] = (update['MinCVaR'].values) / sum(update['MinCVaR'].values) # new weights according to Update and ensures it is 100%
  # En nuevo nominal sumamos el resultado obtenido mas el remanente liquido para reinvertir, siendo nuestro total disponible
  df['nominalNew'] = ((df['weightsNew'] * (df['notionalToday'] + df['oldLiquidity'])) // df['priceToday']) # nuevo nominal
