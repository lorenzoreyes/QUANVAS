...
def GSPC():
    ...
    df = yahoo.download(lista,start="2016-01-01",end="2021-01-01")["Adj Close"].fillna(method="ffill")
    riskfree = yahoo.download(freeRisk,start="2016-01-01",end="2021-01-01")['Adj Close'].fillna(method='ffill')
...
def Merval():
    ...
    panel = [i[0:4] + '.BA' for i in panel.Símbolo.to_list()]
    df = yahoo.download(panel,period="1y")["Adj Close"]#.fillna(method="ffill")
    df = df.dropna(axis=1,how='all')
    pct = df.pct_change()#.dropna()
    mean = pd.DataFrame(pct.mean(),columns=['Mean'],index=pct.columns)
    riskpct = mean.mean()
