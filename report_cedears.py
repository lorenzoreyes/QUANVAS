...
excel = excel.rename(columns={'Cedears Rate': 'CEDEARS','ADRs Rate': 'ADRS', 'Dollar+Taxes':'DOLAR'})

html_file = excel.to_html(na_rep = "").replace('<th>','<th  style="background-color:rgb(60,179,113); color:black">').replace('<table>','<table style="width:100%">')
...
clients = clients.drop_duplicates('Email', keep='first')
hoy = dt.date.today().strftime('%d-%m-%Y')
...
tabla = """ <br><h2>Resumen de precios promedios últimos 12 meses</h2><br>"""
firma = """<h2>Esperamos que esta minuta lo mantenga informado.</h2>
            <h2>Sin más saludamos, equipo QUANVAS.</h2> """

tosend = preview + plotForex + tabla + html_file + firma
...
for i in range(len(clients)):    
  ...
  message = style + highlight + morning + tosend + end_html 
  
  if i == 0:
     e = open('templateCEDEARS.html','w') 
     e.write(message)
     e.close()
