...
data = yahoo.download(lista,period="7d",interval="1m")["Adj Close"].fillna(method="ffill")
...
def chains(chores):
    for i in range(chores,len(clients),5):
      cartera = pd.read_excel(clients.Path.values[i])
      file_path = str(clients.Path.values[i])
      ...
      destinatarios = ['akirafierro@gmail.com']#, f'{clients.Email.values[i]}']
...
if __name__ == '__main__':
    p1 = Process(target=chains(0))
    p2 = Process(target=chains(1))
    p3 = Process(target=chains(2))
    p4 = Process(target=chains(3))
    p5 = Process(target=chains(4))
    p5.start(),p1.start(),p2.start(),p3.start(),p4.start()
